 ...
 */
public class ParameterConveter {

    @SuppressWarnings("unchecked")
    public static <T> T convert(Class<T> requiredType, Object convertedValue) {
        ...
        throw new RequestException("Type for RequestParam '" + requiredType.getName() + "' is not supported");
    }

    private static Boolean convertToBoolean(String source) {
    ...
    private static Character convertToCharacter(String source) {
        if (source.length() == 0) {
            return null;
        }
        if (source.length() > 1) {
